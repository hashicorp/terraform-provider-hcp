...
If an invalid bucket name is provided in the `for_each` map, an error will be thrown. This helps to ensure that the configuration doesn't contain orphaned values. If this behavior is undesirable, filter out invalid buckets from the result of the `merge` function.
