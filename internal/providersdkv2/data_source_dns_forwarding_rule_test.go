...
func TestAccDNSForwardingRuleDataSource(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDNSForwardingRuleDataSourceConfig(),
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.hcp_dns_forwarding_rule.test", "domain_name", "additional.example.com"),
					resource.TestCheckResourceAttr("data.hcp_dns_forwarding_rule.test", "inbound_endpoint_ips.#", "2"),
					resource.TestCheckResourceAttr("data.hcp_dns_forwarding_rule.test", "inbound_endpoint_ips.0", "10.0.1.20"),
					resource.TestCheckResourceAttr("data.hcp_dns_forwarding_rule.test", "inbound_endpoint_ips.1", "10.0.1.21"),
				),
			},
		},
	})
...
func testAccDNSForwardingRuleDataSourceConfig() string {
	return `
...
resource "hcp_dns_forwarding_rule" "test" {
  ...
  domain_name          = "additional.example.com"
...
data "hcp_dns_forwarding_rule" "test" {
  hvn_id            = hcp_hvn.test.hvn_id
  dns_forwarding_id = hcp_dns_forwarding.test.dns_forwarding_id
  rule_id           = hcp_dns_forwarding_rule.test.rule_id
}
`
}
