...
func (f *policyFuture) executeModifers(ctx context.Context, u ResourceIamUpdater, client *clients.Client) {
	...
	// Apply the policy
	f.set(u.SetResourceIamPolicy(ctx, policyMapToPolicy(ep.Etag, bindings)))
...
// policyToMap to map converts an IAM policy to a set of maps. The first map is
// keyed by Role ID, and the second map is keyed by PrincipalID.
func policyToMap(p *models.HashicorpCloudResourcemanagerPolicy) map[string]map[string]*models.HashicorpCloudResourcemanagerPolicyBindingMemberType {
...
// policyMapToPolicy converts the map generated by policyToMap to an IAM Policy
// object.
func policyMapToPolicy(etag string, bindings map[string]map[string]*models.HashicorpCloudResourcemanagerPolicyBindingMemberType) *models.HashicorpCloudResourcemanagerPolicy {
