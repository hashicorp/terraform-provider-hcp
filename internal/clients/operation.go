	...
	for {
		...
		shouldBreak, err := func() (bool, error) {
			...
			if err != nil {
				...
				log.Printf("[INFO] Received state of %s operation (%s): %s", operationName, operationID, *waitResponse.Payload.Operation.State.Pointer())
				if *waitResponse.Payload.Operation.State.Pointer() == *sharedmodels.HashicorpCloudOperationOperationStateDONE.Pointer() {
